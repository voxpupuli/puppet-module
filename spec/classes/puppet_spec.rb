...
describe 'puppet', :type => 'class' do
  ...
  context 'with manage_repo disabled' do
    ...
    it { is_expected.to compile.with_all_deps }
    it { is_expected.to contain_class('puppet') }
    it { is_expected.to_not contain_yumrepo('puppet5') }
    it { is_expected.to_not contain_yumrepo('puppetlabs-pc1') }
  end
  ...
  context 'with invalid loglevel' do
    ...
    it { is_expected.to compile.and_raise_error(/Invalid value "annoying". Valid values are/) }
  end
